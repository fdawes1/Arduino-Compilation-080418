/*  HC12 Send/Receive Example Program 2
    ...
    chage settings on a remote receiver.
    ...
    Do not power HC12 via Arduino over USB.  Per data sheet,
    power HC12 with a supply of at least 100 mA with
    a 22 uF - 1000 uF reservoir capacitor and connect 1N4007 diode in 
    series with HC12 transceiver if potential difference exceeds 4.5 V

    Upload code to two Arduinos connected to two computers 
    that are several meters (required to function) apart.
...
void loop() {
  ...
  while (Serial.available()) {                  // If Arduino's computer rx buffer has data
    ...
    if (SerialByteIn == '\n') {                 // Check to see if at the end of the line
      SerialEnd = true;                         // Set SerialEnd flag to to indicate end of line
    }
  }
